...
GSprite::GSprite(void) : _pVertexBuffer(nullptr), _pIndexBuffer(nullptr), _pTexture(nullptr)
{
}
							...
							int spriteWidth, int spriteHeight)
{
	...
	if(!result)
	{
		DBG_OUTPT("GSprite - cannot initialize buffers");
		return false;
	}
	...
	if(!result)
	{
		DBG_OUTPT("GSprite - cannot load texture");
		return false;
	}
	return true;
}

bool GSprite::render(ID3D11DeviceContext* deviceContext, int positionX, int positionY)
{
	// Re-build the dynamic vertex buffer for rendering to possibly a different location on the screen.
	bool result = updateBuffers(deviceContext, positionX, positionY);
	if(!result)
	{
		DBG_OUTPT("GSprite::render - cannot update buffers");
		return false;
	}
	...
	return true;
												...
												// private
bool GSprite::initializeBuffers(ID3D11Device* device)
{
	...
	if(FAILED(result))
	{
		DBG_OUTPT("GSprite::initializeBuffers - failed creating vertex buffer HRESULT = " << result);
		return false;
	}
	...
	if(FAILED(result))
	{
		DBG_OUTPT("GSprite::initializeBuffers - failed creating index buffer HRESULT = " << result);
		return false;
	}
...
bool GSprite::updateBuffers(ID3D11DeviceContext* deviceContext, int positionX, int positionY)
{
	...
	if( FAILED(result) )
	{
		DBG_OUTPT("GSprite::updateBuffers - failed loking the vertex buffer HRESULT = "<<result);
		return false;
	}
...
void GSprite::renderBuffers(ID3D11DeviceContext* deviceContext)
{
