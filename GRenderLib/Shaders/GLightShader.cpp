#include "GLightShader.h"

GLightShader::GLightShader() : GAbstractShader(), _pSampleState(nullptr), _pLightBuffer(nullptr)
{
}

bool GLightShader::initialize(ID3D11Device* device, HWND hWnd)
{
	bool result = initializeShader(device,hWnd,"shaders/light.vs","shaders/light.ps");
	if(!result)
	{
		DBG_OUTPT("Failed to initialize Light Shader");
		return false;
	}
	return true;
}

bool GLightShader::render(ID3D11DeviceContext* deviceContext, int indexCount,
						...
						const GVector3& lightDirection, const GColor& diffuseColor,const GColor& ambientColor)
{
	bool result = setShaderParameters(deviceContext,world,view,projection,
									texturePtr,lightDirection,diffuseColor,ambientColor);
	if(!result)
	{
		return false;
	}
	...
	return true;
												...
												// protected
void GLightShader::unloadContent()
{
...
bool GLightShader::initializeShader(ID3D11Device* device, HWND hWnd, char* vsFileName, char* psFileName)
{
	...
	if(FAILED(result))
	{
		DBG_OUTPT("GLightShader::initializeShader - error creating vertex shader HRESULT= " << result);
		return false;
	}
	...
	if(FAILED(result))
	{
		DBG_OUTPT("GLightShader::initializeShader - error creating pixel shader HRESULT= " << result);
		return false;
	}
	...
	if(FAILED(result))
	{
		DBG_OUTPT("GLightShader::initializeShader - error creating vertex input layout HRESULT= " << result);
		return false;
	}
	...
	if(FAILED(result))
	{
		DBG_OUTPT("GLightShader::initializeShader - error creating texture sampler state HRESULT= " << result);
		return false;
	}
	...
	if(FAILED(result))
	{
		DBG_OUTPT("GTextureShader::initializeShader - error creating light buffer HRESULT= " << result);
		return false;
	}
	return true;
}
bool GLightShader::setShaderParameters(ID3D11DeviceContext* deviceContext,
									   const GMatrix4x4& world, const GMatrix4x4& view, const GMatrix4x4& projection,
							ID3D11ShaderResourceView* texturePtr,
							const GVector3& lightDirection, const GColor& diffuseColor, const GColor& ambientColor)
{
	...
	if(FAILED(result))
	{
		DBG_OUTPT("GLightShader::setShaderParameters - error mapping matrix subresource HRESULT= " << result);
		return false;
	}
	...
	if(FAILED(result))
	{
		DBG_OUTPT("GLightShader::setShaderParameters - error mapping light subresource HRESULT= " << result);
		return false;
	}
...
void GLightShader::renderShader(ID3D11DeviceContext* deviceContext, int indexCount)
{
