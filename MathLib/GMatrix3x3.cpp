...
GVector3 GMatrix3x3::operator*(const GVector3& v) const
{
	GVector3 r;
	r[0] = (v[0] * _m[0]) + (v[1] * _m[1]) + (v[2] * _m[2]);
	r[1] = (v[0] * _m[3]) + (v[1] * _m[4]) + (v[2] * _m[5]);
	r[2] = (v[0] * _m[6]) + (v[1] * _m[7]) + (v[2] * _m[8]);
	return r;
}
/*
GVector3 GMath::operator*(const GVector3& v, const GMatrix3x3& mat)		//faster then using AMP
...
{	
	...
	*/
	inv(1, 1) = ((operator()(2, 2) * operator()(3, 3)) - (operator()(3, 2)*operator()(2, 3)))*k;
	inv(2, 1) = -((operator()(2, 1) * operator()(3, 3)) - (operator()(2, 3)*operator()(3, 1)))*k;
	inv(3, 1) = ((operator()(2, 1) * operator()(3, 2)) - (operator()(3, 1)*operator()(2, 2)))*k;

	inv(1, 2) = -((operator()(1, 2) * operator()(3, 3)) - (operator()(1, 3)*operator()(3, 2)))*k;
	inv(2, 2) = ((operator()(1, 1) * operator()(3, 3)) - (operator()(1, 3)*operator()(3, 1)))*k;
	inv(3, 2) = -((operator()(1, 1) * operator()(3, 2)) - (operator()(3, 1)*operator()(1, 2)))*k;

	inv(1, 3) = ((operator()(1, 2) * operator()(2, 3)) - (operator()(1, 3)*operator()(2, 2)))*k;
	inv(2, 3) = -((operator()(1, 1) * operator()(2, 3)) - (operator()(2, 1)*operator()(1, 3)))*k;
	inv(3, 3) = ((operator()(1, 1) * operator()(2, 2)) - (operator()(2, 1)*operator()(1, 2)))*k;
	return inv;
}
...
GMatrix3x3 GMatrix3x3::AntiSymmetricMatrix(const GVector3& v)
{
	GMatrix3x3 M;
	M._m[0] = 0.0f;
	M._m[1] = -1.0f*v.z();
