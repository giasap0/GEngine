...
public:
	...
	inline Real& operator()(uint row, uint column)				{row-=1;column-=1;return _m[column + (row*3) ];}
	//(first element==11 not 00)
	inline const Real& operator()(uint row, uint column) const			{row-=1;column-=1;return _m[column + (row*3) ];}
	//return Real _m[9]
