...
{
	_applicationName= "Giampaolo Saporito Flight Simulator";
}
...
{	
	...
	// Initialize the physic system. This object will handle the airplane status
	_pPhysic = new GPhysic();
	if (_pPhysic == nullptr)		{ return E_FAIL; }

	res = _pPhysic->initialize();
	if (!res)
	{
		//MessageBox(_hWnd, "Could not initialize the physic system.", "Error", MB_OK);
		GString noPhysic = GString("Failed to initialize the physic system - check the airplane input file for building info ") + GPhysics::GAirplaneLoader::fileName;
		MessageBox(_hWnd, noPhysic.const_data(), "Error", MB_OK);
		DBG_OUTPT( noPhysic.const_data() );
		return E_FAIL;
	}
...
void GSystem::run()
{
	...
	while(msg.message != WM_QUIT && quit == false)
	{
		...
		{
			quit = !_pInput->update();
			_pInput->getMouseLocation(mouseX,mouseY, mouseZ);

			if (_pInput->generalCommand().quit())
			{
				quit = true;
				DBG_OUTPT("ESC pressed, exit from application");
				break;
			}
			if (_pInput->generalCommand().isPause() )
			{
				DBG_OUTPT("pause application");
			...
			_pPhysic->updateState(dt, _pInput->planeCommand() );		
			logger.logAirplaneStatus(dt, _pPhysic->getPlaneStatus() );

			_pGraphics->setFps(_pFps->getFps() );
			_pGraphics->updateState(dt, mouseX, mouseY, mouseZ, _pInput->cameraCommand(), _pPhysic->getPlaneStatus());
